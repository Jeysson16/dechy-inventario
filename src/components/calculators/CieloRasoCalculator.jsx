...
const PRODUCTS = {
  mainTee: {
    name: "Te Principal",
    dim: "32×24×3660×0.30mm",
    length: 3.66,
    unit: "pza",
  },
  longCross: {
    name: "Te Secundaria",
    dim: "25×24×1220×0.30mm",
    length: 1.22,
    unit: "pza",
  },
  shortCross: {
    name: "Te Terciaria",
    dim: "25×24×610×0.30mm",
    length: 0.61,
    unit: "pza",
  },
  wallAngle: {
    name: "Ángulo Perimetral",
    dim: "22×19×3000×0.4mm",
    length: 3.0,
    unit: "pza",
  },
...
// ─── Box/Unit Breakdown ───────────────────────────────────────────────────────
function boxBreakdown(qty, unitsPerBox) {
  if (!unitsPerBox || unitsPerBox <= 1) return { boxes: 0, units: qty };
  return { boxes: Math.floor(qty / unitsPerBox), units: qty % unitsPerBox };
}

// ─── Calculation Engine ───────────────────────────────────────────────────────
function calculate(
  { largo, ancho, tileType, system, waste, centered },
  prices = UNIT_PRICES,
) {
  ...
  const { w: tileW, h: tileH } =
    TILE_OPTIONS[tileType] || TILE_OPTIONS["603x603"];
  ...
  const cutTiles = Math.max(
    0,
    rowCount * edgeCols +
      colCount * edgeRows -
      (edgeCols * edgeRows > 0 ? 4 : 0),
  );
  ...
  const shortCrossPositions = Math.max(
    0,
    Math.ceil(L / MODULE) - longCrossPositions,
  );
  ...
  // ── Cost ──────────────────────────────────────────────────────────────────
  const estimatedCost =
    totalTiles * prices.tile +
    totalMainTees * prices.mainTee +
    totalLongCross * prices.longCross +
    totalShortCross * prices.shortCross +
    totalWallAngles * prices.wallAngle +
    totalHangers * prices.hanger +
    totalWire * prices.wire +
    totalAnchors * prices.anchor;
  ...
  return {
    ...
    totalMainTees,
    mainTeeLines,
    mainTeePiecesPerLine,
    totalLongCross,
    longCrossPositions,
    longCrossPerPosition,
    totalShortCross,
    shortCrossPositions,
    shortCrossPerPosition,
    totalWallAngles,
    totalHangers,
    totalWire,
    totalAnchors,
    ...
    wastePercent:
      tilesExact > 0
        ? Math.round(((totalTiles - tilesExact) / tilesExact) * 100)
        : 0,
  };
}
...
function buildFloorPlanData(inputs, results, scale) {
  ...
  for (let i = 0; i <= Math.ceil(A / tileW); i++) {
    ...
    mainTeeLines.push({
      x1: Math.min(x, A * scale),
      y1: 0,
      x2: Math.min(x, A * scale),
      y2: L * scale,
    });
  }
  ...
  for (let j = 0; j <= Math.ceil(L / tileH); j++) {
    ...
    crossTeeLines.push({
      x1: 0,
      y1: Math.min(y, L * scale),
      x2: A * scale,
      y2: Math.min(y, L * scale),
    });
  }
...
function FloorPlanSVG({ inputs, results, colors }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-3">
        ...
        <p className="text-sm font-medium">
          Ingrese dimensiones para ver el plano
        </p>
      </div>
    );
  }
  ...
  const { tiles, mainTeeLines, crossTeeLines, roomW, roomH } =
    buildFloorPlanData(inputs, results, scale);
    ...
    >
      ...
      {crossTeeLines.map((l, i) => (
        <line
          ...
          x2={ox + l.x2}
          y2={oy + l.y2}
          stroke={i % 2 === 0 ? colors.cross1 : colors.cross2}
      ...
      {mainTeeLines.map((mt, mi) =>
        crossTeeLines
          ...
          .map((ct, ci) => {
            const px = ox + mt.x1,
              py = oy + ct.y1;
            if (px < ox || px > ox + roomW || py < oy || py > oy + roomH)
              return null;
            return (
              <circle
                key={`h${mi}-${ci}`}
...
function drawPDFFloorPlan(doc, inputs, results, startX, startY, maxW, maxH) {
  ...
  doc.text(`${L.toFixed(2)} m`, ox - 5, oy + roomH / 2, {
    align: "center",
    angle: 90,
  });
  ...
  doc.text(`${(L * A).toFixed(1)}m²`, ox + roomW / 2, oy + roomH / 2 + 2, {
    align: "center",
  });
  ...
  if (lx + 50 < startX + maxW + 55) {
    ...
    doc.setTextColor(71, 85, 105);
    [
      [99, 102, 241, "T.Principal"],
      [129, 140, 248, "T.Secundaria"],
      [196, 181, 253, "T.Terciaria"],
      [249, 115, 22, "Corte"],
    ].forEach(([r, g, b, label], idx) => {
...
async function exportExcel(
  ...
  s1.getColumn("A").width = 30;
  s1.getColumn("B").width = 22;
  s1.getRow(1).height = 32;
  const titleCell = s1.getCell("A1");
  titleCell.value = "CALCULADORA DE CIELO RASO MODULAR — JieDa S.A.C";
  ...
  [
    ...
    ["Sistema", inputs.system],
    ["Tipo Baldosa", inputs.tileType],
    ["% Desperdicio", `${inputs.waste}%`],
    ["Altura", `${inputs.height} m`],
  ].forEach(([k, v]) => {
  ...
  const totalRow = s1.addRow([
    ...
    `S/ ${results.estimatedCost.toFixed(2)}`,
  ]);
  totalRow.eachCell((c) => {
  ...
  const rows2 = [
    ...
    [
      "Te Principal",
      PRODUCTS.mainTee.dim,
      results.totalMainTees,
      "pza",
      prices.mainTee,
      prices.mainTee > 0,
    ],
    [
      "Te Secundaria",
      PRODUCTS.longCross.dim,
      results.totalLongCross,
      "pza",
      prices.longCross,
      prices.longCross > 0,
    ],
    [
      "Te Terciaria",
      PRODUCTS.shortCross.dim,
      results.totalShortCross,
      "pza",
      prices.shortCross,
      prices.shortCross > 0,
    ],
    [
      "Ángulo Perimetral",
      PRODUCTS.wallAngle.dim,
      results.totalWallAngles,
      "pza",
      prices.wallAngle,
      prices.wallAngle > 0,
    ],
    ["Suspensores", "Con clip metálico", results.totalHangers, "u", 0, false],
    ["Alambre galvanizado", "Calibre 16", results.totalWire, "m", 0, false],
    [
      "Anclajes / Tarugos",
      "Fijación techo",
  ...
  rows2.forEach(([name, spec, qty, unit, price, hasPrice], idx) => {
    const r = s2.addRow([
      name,
      spec,
      qty,
      unit,
      hasPrice ? price.toFixed(2) : "Por registrar",
      hasPrice ? (qty * price).toFixed(2) : "S/ 0.00",
    ]);
    r.height = 18;
  ...
  s2
    .addRow([
      "* Los precios son referenciales. Validar con proveedor antes de adquirir.",
    ])
    .getCell(1).font = {
    italic: true,
    size: 8,
  ...
  s2
    .addRow([
      "Calculadora propiedad de JieDa S.A.C \u2014 Todos los derechos reservados",
    ])
    .getCell(1).font = {
    italic: true,
    size: 8,
  ...
  // Title
  s3.getCell(1, 1).value =
    `Plano Visual – ${A}m × ${L}m | Baldosa: ${inputs.tileType} | Sistema: ${inputs.system}`;
  ...
  for (let row = 0; row < rowCount; row++) {
    ...
    for (let col = 0; col < colCount; col++) {
      ...
      cell.border = {
        top: {
          style: "thin",
          color: { argb: isCut ? "FFF97316" : "FFE2E8F0" },
        },
        left: {
          style: isCut ? "thin" : "hair",
          color: { argb: isCut ? "FFF97316" : "FFCBD5E1" },
        },
        bottom: {
          style: "thin",
          color: { argb: isCut ? "FFF97316" : "FFE2E8F0" },
  ...
  [
    [white, "Baldosa completa"],
    [orange, "Baldosa cortada (corte)"],
    [lightBlue, "Posición Te Principal"],
    [indigo, "Te Secundaria"],
...
// ─── PDF Export ───────────────────────────────────────────────────────────────
function exportPDF(inputs, results, prices = UNIT_PRICES, logoB64 = null) {
  if (!results) return;
  ...
  if (logoB64) {
    try {
      doc.addImage(logoB64, "PNG", PW - M - 26, 2, 22, 22);
    } catch (_) {}
  }
  ...
  doc.text(
    `Sistema: ${inputs.system}  |  Baldosa: ${inputs.tileType}  |  ${new Date().toLocaleDateString("es-PE")}`,
    M,
    20,
  );
  ...
  autoTable(doc, {
    startY: 22,
    head: [
      ["Material", "Especificaciones", "Cant.", "Unid.", "P.Unit.", "Total"],
    ],
    body: [
      [
        PRODUCTS.tile.name,
        ...
        results.totalTiles,
        "u",
        prices.tile > 0 ? `S/${prices.tile}` : "Por registrar",
        prices.tile > 0
          ? `S/${(results.totalTiles * prices.tile).toFixed(2)}`
          : "S/ 0.00",
      ],
      [
        PRODUCTS.mainTee.name,
        PRODUCTS.mainTee.dim,
        results.totalMainTees,
        "pza",
        prices.mainTee > 0 ? `S/${prices.mainTee}` : "Por registrar",
        prices.mainTee > 0
          ? `S/${(results.totalMainTees * prices.mainTee).toFixed(2)}`
          : "S/ 0.00",
      ],
      [
        PRODUCTS.longCross.name,
        PRODUCTS.longCross.dim,
        results.totalLongCross,
        "pza",
        prices.longCross > 0 ? `S/${prices.longCross}` : "Por registrar",
        prices.longCross > 0
          ? `S/${(results.totalLongCross * prices.longCross).toFixed(2)}`
          : "S/ 0.00",
      ],
      [
        PRODUCTS.shortCross.name,
        PRODUCTS.shortCross.dim,
        results.totalShortCross,
        "pza",
        prices.shortCross > 0 ? `S/${prices.shortCross}` : "Por registrar",
        prices.shortCross > 0
          ? `S/${(results.totalShortCross * prices.shortCross).toFixed(2)}`
          : "S/ 0.00",
      ],
      [
        PRODUCTS.wallAngle.name,
        PRODUCTS.wallAngle.dim,
        results.totalWallAngles,
        "pza",
        prices.wallAngle > 0 ? `S/${prices.wallAngle}` : "Por registrar",
        prices.wallAngle > 0
          ? `S/${(results.totalWallAngles * prices.wallAngle).toFixed(2)}`
          : "S/ 0.00",
      ],
      [
        "Suspensores",
        "Con clip metálico",
        results.totalHangers,
        "u",
        "Por registrar",
        "S/ 0.00",
      ],
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    "Precios actualizados. Se recomienda validar con la tienda local antes de adquirir materiales.",
    M + 4,
    dY + 6,
  );
  doc.text(
    "Los cálculos son estimaciones basadas en fórmulas estándar de la industria.",
    M + 4,
    dY + 11,
  );
  ...
  doc.text("Calculadora de Cielo Raso Modular", PW / 2, PH - 6, {
    align: "center",
  });
...
export default function CieloRasoCalculator({
  isModal = false,
  onClose,
  showQR = false,
}) {
  ...
  const [firestorePrices, setFirestorePrices] = useState(UNIT_PRICES);
  ...
  // Load logo
  useEffect(() => {
    fetch("/img/brand/logopngjieda.png")
      .then((r) => r.blob())
      .then((blob) => {
  ...
  // Load QR code for shop calculator URL (only in system context)
  useEffect(() => {
    if (!showQR) return;
    const url = `${window.location.origin}/tienda/calculadora`;
  ...
  // Load prices from Firestore — fetch all products and match by name keywords
  useEffect(() => {
    const norm = (s) =>
      (s || "")
        .toLowerCase()
    ...
    getDocs(collection(db, "products"))
      .then((snap) => {
        const prices = { ...UNIT_PRICES };
        const boxData = {};
        snap.forEach((docSnap) => {
          const d = docSnap.data();
          const name = norm(d.name);
          let key = null;
          if (name.includes("baldosa")) key = "tile";
          else if (name.includes("principal")) key = "mainTee";
          else if (name.includes("secundaria")) key = "longCross";
          else if (name.includes("terciaria")) key = "shortCross";
          else if (name.includes("angulo") || name.includes("perimetral"))
            key = "wallAngle";
          if (key) {
            const up = Number(d.unitPrice) || Number(d.price) || prices[key];
            const upb = Number(d.unitsPerBox) || 1;
            const bp = Number(d.boxPrice) || up * upb;
            prices[key] = up;
            boxData[key] = { unitPrice: up, boxPrice: bp, unitsPerBox: upb };
          }
        });
        setFirestorePrices(prices);
        setProductBoxData(boxData);
      })
      .catch(() => {});
  }, []);

  const results = useMemo(
    () => calculate(inputs, firestorePrices),
    [inputs, firestorePrices],
  );
  ...
  const handleExcelExport = async () => {
    setExporting(true);
    try {
      await exportExcel(inputs, results, firestorePrices, logoB64);
    } catch (e) {
      console.error(e);
    } finally {
      setExporting(false);
  ...
  return (
    <div className={containerCls}>
      <div className={cardCls}>
        {/* ─── Header ──────────────────────────────────────────────────── */}
        <div
          className={`flex items-center justify-between px-4 sm:px-6 py-3 shrink-0 bg-gradient-to-r ${colors.bg}`}
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base font-black text-white leading-tight truncate">
                Calculadora Cielo Raso Modular
              </h2>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {results && (
              <>
                <button
                  onClick={() =>
                    exportPDF(inputs, results, firestorePrices, logoB64)
                  }
                    ...
                    >
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth={2}
                          d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h2M4 4h4v4H4V4zm0 12h4v4H4v-4zm12-12h4v4h-4V4z"
                        />
                      </svg>
                      <span className="hidden sm:inline">QR</span>
                    </button>
                    {showQRPanel && (
                      <div className="absolute right-0 top-full mt-2 z-50 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-4 flex flex-col items-center gap-3 min-w-[180px]">
                        ...
                        <img
                          src={qrDataURL}
                          alt="QR Calculadora"
                          className="size-36 rounded-lg border border-slate-200"
                        />
                        <a
                          href={qrDataURL}
                          download="calculadora-jieda-qr.png"
                          className="w-full text-center px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-black uppercase transition-all"
                        >
                          Descargar QR
                        </a>
                      </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(TILE_OPTIONS).map(([key, { label }]) => (
                      ...
                      >
                        <div
                          className={`text-xs font-black ${inputs.tileType === key ? "text-indigo-600" : "text-slate-700 dark:text-slate-300"}`}
                        >
                          {label}
                        </div>
                    ...
                    {Object.entries(SYSTEM_COLORS).map(([sys, c]) => (
                      ...
                      >
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <div
                            className={`size-2 rounded-full bg-gradient-to-br ${c.bg}`}
                          ></div>
                          <span
                            className={`text-[11px] font-black ${inputs.system === sys ? "text-indigo-600" : "text-slate-700 dark:text-slate-300"}`}
                          >
                            {sys}
                          </span>
                        </div>
                ...
                <div className="flex items-center justify-between p-3.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Simetría desde el centro del ambiente
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => set("centered", !inputs.centered)}
                  ...
                  >
                    <span
                      className={`inline-block size-5 transform rounded-full bg-white shadow-sm transition-transform ${inputs.centered ? "translate-x-5" : "translate-x-0.5"}`}
                    />
                  </button>
                </div>
                ...
                {results ? (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        {
                          l: "Área Total",
                          v: `${results.area.toFixed(2)} m²`,
                          s: `${results.colCount}×${results.rowCount} módulos`,
                          c: "indigo",
                        },
                        {
                          l: "Perímetro",
                          v: `${results.perimeter.toFixed(1)} m`,
                          s: "Linear",
                          c: "sky",
                        },
                        {
                          l: "Baldosas",
                          v: `${results.totalTiles} u`,
                          s: `+${results.wastePercent}% desp.`,
                          c: "violet",
                        },
                        {
                          l: "Costo Estimado",
                          v: `S/ ${results.estimatedCost.toFixed(0)}`,
                          s: "Referencial",
                          c: "emerald",
                        },
                      ].map(({ l, v, s, c }) => (
                        <div
                          key={l}
                          className={`p-3.5 rounded-2xl bg-${c}-50 dark:bg-${c}-900/20 border border-${c}-100 dark:border-${c}-900/30`}
                        >
                          <div
                            className={`text-base font-black text-${c}-700 dark:text-${c}-400 leading-tight`}
                          >
                            {v}
                          </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      {[
                        {
                          l: `Te Principal (${PRODUCTS.mainTee.dim})`,
                          v: results.totalMainTees,
                          u: "pzas",
                          bk: "mainTee",
                        },
                        {
                          l: `Te Secundaria (${PRODUCTS.longCross.dim})`,
                          v: results.totalLongCross,
                          u: "pzas",
                          bk: "longCross",
                        },
                        {
                          l: `Te Terciaria (${PRODUCTS.shortCross.dim})`,
                          v: results.totalShortCross,
                          u: "pzas",
                          bk: "shortCross",
                        },
                        {
                          l: `Ángulo Perimetral (${PRODUCTS.wallAngle.dim})`,
                          v: results.totalWallAngles,
                          u: "pzas",
                          bk: "wallAngle",
                        },
                        {
                          l: "Suspensores",
                          v: results.totalHangers,
                          u: "u",
                          bk: null,
                        },
                        {
                          l: "Alambre galvanizado",
                          v: results.totalWire,
                          u: "m",
                          bk: null,
                        },
                        {
                          l: "Anclajes",
                          v: results.totalAnchors,
                          u: "u",
                          bk: null,
                        },
                      ].map(({ l, v, u, bk }) => {
                        const upb = bk
                          ? productBoxData[bk]?.unitsPerBox || 1
                          : 1;
                        const bd = upb > 1 ? boxBreakdown(v, upb) : null;
                        return (
                          <div
                            key={l}
                            className="flex justify-between items-center px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50"
                          >
                            <span className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 truncate mr-2">
                              {l}
                            </span>
                            <div className="text-right shrink-0">
                              ...
                              {bd && bd.boxes > 0 && (
                                <div className="text-[9px] text-indigo-500 dark:text-indigo-400 font-bold">
                                  {bd.boxes} caj. + {bd.units} und.
                                </div>
                              )}
                            </div>
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-full min-h-[280px] text-slate-300 dark:text-slate-600 gap-3">
                    ...
                    <p className="text-sm font-semibold text-center">
                      Ingresa largo y ancho para calcular
                    </p>
                  </div>
                )}
              </div>
                  ...
                  {[
                    {
                      ...
                      label: "Baldosa completa (603×603mm)",
                    },
                    {
                      color: "bg-orange-50 border-orange-400",
                      label: "Baldosa cortada",
                    },
                    {
                      color: "bg-indigo-500",
                      label: `Te Principal (${PRODUCTS.mainTee.dim})`,
                    },
                    {
                      color: "bg-indigo-300",
                      label: `Te Secundaria (${PRODUCTS.longCross.dim})`,
                    },
                    {
                      color: "bg-indigo-100",
                      label: `Te Terciaria (${PRODUCTS.shortCross.dim})`,
                    },
                    {
                      color: "bg-slate-400",
                      label: `Ángulo Perimetral (${PRODUCTS.wallAngle.dim})`,
                      dashed: true,
                    },
                    {
                      color: "bg-red-400",
                      label: "Punto de suspensión",
                      circle: true,
                    },
                  ].map(({ color, label, dashed, circle }) => (
                    <div key={label} className="flex items-center gap-2">
                      <div
                        className={`${circle ? "rounded-full size-3" : "rounded size-3"} ${color} border shrink-0 ${dashed ? "border-dashed" : ""}`}
                      ></div>
              ...
              {results ? (
                <div className="space-y-5">
                    ...
                    <table className="w-full text-left min-w-[640px]">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/50">
                          {[
                            ...
                            <th
                              key={h}
                              className="px-3 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400"
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {[
                          {
                            p: PRODUCTS.tile,
                            qty: results.totalTiles,
                            key: "tile",
                            c: "violet",
                          },
                          {
                            p: PRODUCTS.mainTee,
                            qty: results.totalMainTees,
                            key: "mainTee",
                            c: "indigo",
                          },
                          {
                            p: PRODUCTS.longCross,
                            qty: results.totalLongCross,
                            key: "longCross",
                            c: "sky",
                          },
                          {
                            p: PRODUCTS.shortCross,
                            qty: results.totalShortCross,
                            key: "shortCross",
                            c: "blue",
                          },
                          {
                            p: PRODUCTS.wallAngle,
                            qty: results.totalWallAngles,
                            key: "wallAngle",
                            c: "slate",
                          },
                          {
                            p: {
                              name: "Suspensores",
                              dim: "Con clip metálico",
                              unit: "u",
                            },
                            qty: results.totalHangers,
                            key: null,
                            c: "amber",
                          },
                          {
                            ...
                            qty: results.totalWire,
                            key: null,
                            c: "slate",
                          },
                          {
                            p: {
                              name: "Anclajes/Tarugos",
                              dim: "Fijación techo",
                              unit: "u",
                            },
                            qty: results.totalAnchors,
                            key: null,
                            c: "rose",
                          },
                        ].map(({ p, qty, key, c }) => {
                          const price = key ? firestorePrices[key] : 0;
                          const bd = key
                            ? boxBreakdown(
                                qty,
                                productBoxData[key]?.unitsPerBox || 1,
                              )
                            : null;
                          const desgloseText =
                            bd && bd.boxes > 0
                          ...
                          return (
                            ...
                            >
                              <td className="px-3 py-2.5 text-sm font-black text-slate-900 dark:text-white">
                                {p.name}
                              </td>
                              ...
                              <td className="px-3 py-2.5">
                                <span
                                  className={`text-sm font-black text-${c}-600 dark:text-${c}-400`}
                                >
                                  {qty}
                                </span>
                              </td>
                      ...
                      <tfoot>
                        <tr className="bg-emerald-50 dark:bg-emerald-900/20 border-t-2 border-emerald-200 dark:border-emerald-900/30">
                          <td
                            colSpan={5}
                            className="px-3 py-3.5 text-sm font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-wide"
                          >
                            Total Estimado
                          </td>
                          <td className="px-3 py-3.5 text-lg font-black text-emerald-700 dark:text-emerald-400">
                            S/ {results.estimatedCost.toFixed(2)}
                          </td>
                        </tr>
                      </tfoot>
                  ...
                  <div className="p-3.5 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-100 dark:border-amber-900/30 flex gap-3">
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                      Precios referenciales. Los cálculos son estimaciones —
                      validar cantidades con instalador profesional.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-20 text-slate-300 dark:text-slate-600 gap-3">
                  ...
                  <p className="text-sm font-semibold">
                    Completa los datos para ver los materiales
                  </p>
                </div>
              )}
            </div>
