...
const InventoryList = () => {
  ...
  const toggleLocation = (shelfIdx, rowIdx, col, levelIdx = 0) => {
    ...
    if (activeLayout.id === (branchLayouts[0]?.id || "main")) {
      if (tempLocations[legacyKey] !== undefined && levelIdx === 0) {
        ...
        !Object.keys(tempLocations).some((k) =>
          k.startsWith(`${activeLayout.id}__`),
        )
      ) {
    ...
    if (
      activeLayout.id === branchLayouts[0]?.id &&
      tempLocations[baseKey] !== undefined
    ) {
      key = baseKey;
    }
  ...
  const saveLocations = async () => {
    ...
    const unitsPerBox = Number(selectedProductForLocation.unitsPerBox) || 1;
    const totalAvailableUnits =
      selectedProductForLocation.currentStock * unitsPerBox +
      Number(selectedProductForLocation.remainderUnits || 0);
    const totalAssigned = Object.values(tempLocations).reduce(
      (a, b) => a + Number(b),
      0,
    );
    if (totalAssigned > totalAvailableUnits) {
      toast.error(
        `No puedes asignar ${totalAssigned} unidades. El stock disponible es de ${totalAvailableUnits} unidades.`,
      );
      return;
    }
  ...
  const renderKPIs = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 px-6 lg:px-10 mt-6">
      {/* Total Products */}
      <div className="flex flex-col gap-3 rounded-[2rem] p-7 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 shadow-xl shadow-slate-200/40 dark:shadow-none hover:shadow-primary/5 transition-all group hover:-translate-y-1 hover:scale-[1.02]">
      ...
      {/* Stock Bajo */}
      <div className="flex flex-col gap-3 rounded-[2rem] p-7 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 shadow-xl shadow-slate-200/40 dark:shadow-none hover:shadow-rose-500/5 transition-all group hover:-translate-y-1 hover:scale-[1.02]">
        <div className="flex items-center justify-between">
      ...
      {/* Valor Total */}
      <div className="flex flex-col gap-3 rounded-[2rem] p-7 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 shadow-xl shadow-slate-200/40 dark:shadow-none hover:shadow-emerald-500/5 transition-all group hover:-translate-y-1 hover:scale-[1.02]">
        <div className="flex items-center justify-between">
        ...
        <div>
          <p className="text-slate-900 dark:text-white text-3xl font-black tracking-tight leading-none h-[40px] flex items-end">
            {loading ? "..." : `S/${(metrics.totalValue / 1000).toFixed(1)}k`}
          </p>
          <div className="flex items-center gap-1.5 mt-2 overflow-hidden">
            <div className="flex items-center gap-1 text-emerald-600 text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 px-2 py-1 rounded-lg">
              <span className="material-symbols-outlined text-[14px]">
      ...
      {/* Total Cajas */}
      <div className="flex flex-col gap-3 rounded-[2rem] p-7 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/50 dark:border-slate-800/50 shadow-xl shadow-slate-200/40 dark:shadow-none hover:shadow-indigo-500/5 transition-all group hover:-translate-y-1 hover:scale-[1.02]">
        <div className="flex items-center justify-between">
  ...
  const renderGrid = (data = filteredProducts) => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {data.map((p, index) => {
        ...
        const hasReferencial = images.some(
          (img) => img.type === "imagen referencial",
        );
        ...
        return (
          ...
          >
            ...
            <div className="absolute top-4 left-4 right-4 z-20 flex flex-wrap gap-2 pointer-events-none">
              ...
              {hasUso && (
                <span className="bg-violet-500/90 text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest backdrop-blur-md border border-violet-400">
                  Uso
                </span>
              )}
            </div>
            ...
            <div className="relative w-full aspect-[1/1] overflow-hidden bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
              ...
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center gap-3 underline-offset-4 backdrop-blur-[2px]">
                <button
                  ...
                  className="size-12 rounded-full bg-white text-slate-900 flex items-center justify-center shadow-xl hover:bg-primary hover:text-white transition-all scale-75 group-hover:scale-100 hover:scale-110 active:scale-95 translate-y-4 group-hover:translate-y-0"
                >
                  <span className="material-symbols-outlined">edit</span>
                </button>
                <button
                  onClick={() => openLocationModal(p)}
                  className="size-12 rounded-full bg-white text-slate-900 flex items-center justify-center shadow-xl hover:bg-primary hover:text-white transition-all scale-75 group-hover:scale-100 hover:scale-110 active:scale-95 translate-y-4 group-hover:translate-y-0 delay-75"
                >
            ...
            <div className="p-8 flex flex-col gap-5 flex-1 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
              <div className="space-y-2">
                ...
                <p className="text-slate-400 dark:text-slate-500 text-[11px] font-black uppercase tracking-widest">
                  {p.brand || "Marca Premium"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="flex flex-col gap-1 text-right">
                  ...
                  <div className="flex items-baseline justify-end gap-1">
                    <span
                      className={`font-black text-xl tracking-tighter ${p.currentStock > 0 ? "text-slate-900 dark:text-white" : "text-rose-500"}`}
                    >
                      {p.currentStock || 0}
                    </span>
              ...
              {Number(p.wholesalePrice) > 0 && (
                <div className="group/wholesale p-4 bg-gradient-to-br from-indigo-50 to-white dark:from-indigo-950/20 dark:to-slate-900 rounded-3xl border border-indigo-100 dark:border-indigo-900/50 flex items-center justify-between transition-all hover:border-primary/30">
                  <div className="flex flex-col">
                    <span className="text-indigo-600 dark:text-indigo-400 font-black uppercase tracking-widest text-[8px]">
                      Precio Mayorista
                    </span>
                  ...
                  <div className="text-right">
                    <span className="text-slate-400 font-black uppercase tracking-widest text-[8px]">
                      Desde
                    </span>
                    <div className="flex items-center justify-end gap-1">
                      <span className="text-slate-700 dark:text-slate-200 font-black text-xs">
                        {p.wholesaleThreshold}
                      </span>
                      <span className="text-slate-400 font-bold text-[9px] uppercase">
                        {p.wholesaleThresholdUnit === "cajas" ? "cjs" : "und"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex -space-x-3">
                  ...
                  activeLayout ? (
                    ...
                    <div className="size-10 rounded-full bg-slate-50 dark:bg-slate-800 border-2 border-slate-50 dark:border-slate-900 flex items-center justify-center shadow-sm">
                      <span className="material-symbols-outlined text-slate-200 text-[18px]">
                        location_off
                      </span>
                    </div>
                  )}
          ...
          <div className="max-w-screen-xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Gestión y control de productos en stock
              </p>
            </div>
          </div>
        </div>
              ...
              {loading ? (
                <div className="flex justify-center py-24">
                  <div className="relative">
                    ...
                    <span className="material-symbols-outlined animate-spin text-5xl text-primary relative">
                      progress_activity
                    </span>
                  </div>
                </div>
                ...
                <DataTable
                  ...
                  headerContent={
                    <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-1 lg:pb-0 custom-scrollbar no-scrollbar">
                      ...
                      >
                        <span className="material-symbols-outlined group-active:rotate-180 transition-transform text-[22px]">
                          refresh
                        </span>
                      </button>
                      ...
                      render: (val, item) => (
                        <div className="flex flex-col gap-0.5 min-w-[150px]">
                          ...
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            {item.brand || "Genérico"} - {item.sku || "SIN SKU"}
                          </span>
                        </div>
                      ),
                    },
                        ...
                        val > 0 ? (
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                              ...
                              {item.wholesaleThresholdUnit === "cajas"
                                ? "cjs"
                                : "und"}
                              )
                            </span>
                  ...
                  actions={[
                    ...
                    },
                    {
                      ...
                      icon: "edit",
                      onClick: (item) =>
                        navigate(`/editar-producto/${item.id}`),
                    },
                    {
                      label: "Eliminar",
                      icon: "delete",
                        ...
                        >
                          ...
                          <td className="py-3 px-2">
                            <div className="flex flex-col gap-0.5">
                              ...
                              {tx.saleId && (
                                <span className="text-[9px] text-slate-400 font-mono pl-3">
                          ...
                          <td className="py-3 px-2">
                            <div className="flex items-center gap-2">
                              <div className="size-6 rounded-full bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center text-[10px] font-black text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800">
                                {(tx.userEmail || tx.user || "?")
                                  .charAt(0)
                                  .toUpperCase()}
                              </div>
                              <div className="flex flex-col">
                                <span
                                  className="text-[10px] font-bold text-slate-600 dark:text-slate-400 truncate max-w-[120px]"
                                  title={tx.userEmail || tx.user}
                                >
              ...
              <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 sticky top-0 z-30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-medium">
                      {selectedProductForLocation?.name}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div
                    className={`px-4 py-2 rounded-xl flex items-center gap-2 border transition-all ${
                      Object.values(tempLocations).reduce(
                        (a, b) => a + Number(b),
                        0,
                      ) >
                      (selectedProductForLocation?.currentStock || 0) *
                        (Number(selectedProductForLocation?.unitsPerBox) || 1) +
                        Number(selectedProductForLocation?.remainderUnits || 0)
                  ...
                  >
                    ...
                    <span className="text-xs font-black">
                      ...
                      {(selectedProductForLocation?.currentStock || 0) *
                        (Number(selectedProductForLocation?.unitsPerBox) || 1) +
                        Number(
                          selectedProductForLocation?.remainderUnits || 0,
                        )}{" "}
                      <span className="text-[10px] opacity-70">unidades</span>
                    </span>
                  </div>
                  <button
                ...
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  ...
                  {clearLocationsUndo && (
                    <div className="flex items-center gap-2 px-3 h-10 rounded-xl bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800/60">
                      ...
                      <button
                        onClick={undoClearLocations}
                        className="flex items-center gap-1 px-2.5 py-1 bg-rose-600 text-white text-xs font-black rounded-lg hover:bg-rose-700 transition-colors"
                      >
                        <span className="material-symbols-outlined text-[14px]">
                          undo
                        </span>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl p-6 w-full max-w-sm mx-4">
            <div className="size-12 rounded-2xl bg-rose-100 dark:bg-rose-900/40 flex items-center justify-center mb-4">
              <span className="material-symbols-outlined text-[24px] text-rose-600 dark:text-rose-400">
                location_off
              </span>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={confirmClearLocations}
                className="flex-1 py-2.5 rounded-xl bg-rose-600 text-white text-sm font-bold hover:bg-rose-700 transition-colors flex items-center justify-center gap-2"
              >
                <span className="material-symbols-outlined text-[18px]">
                  location_off
                </span>
