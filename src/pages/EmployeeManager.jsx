...
const EmployeeManager = () => {
  ...
  const openEditModal = (emp) => {
    ...
    setShowPassword(false);
    setAvatarFile(null);
    setAvatarPreview(emp.avatarUrl || "");
    setUploadProgress(0);
    setIsModalOpen(true);
  };
    ...
    try {
      ...
      if (avatarFile) {
        const storageRef = ref(
          storage,
          `employees/${Date.now()}_${avatarFile.name}`,
        );
      ...
      if (editingEmployee) {
        // Edit: update Firestore doc with new avatar if provided
        const updateData = {
          name: formData.name,
          role: formData.role,
        ...
        if (avatarUrl) {
          employeeData.avatarUrl = avatarUrl;
        }
        await setDoc(doc(db, "employees", newUid), employeeData);

        toast.success(
          `Empleado "${formData.name}" creado. Puede iniciar sesión con ${formData.email}.`,
        );
      }
  ...
  const handleToggleStatus = async (emp) => {
    ...
    try {
      ...
      toast.success(
        `Empleado ${newStatus === "Activo" ? "activado" : "desactivado"}.`,
      );
    } catch (error) {
          ...
          <div className="max-w-screen-xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Administre usuarios, roles y accesos del sistema.
              </p>
            </div>
          ...
          <div className="max-w-screen-xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div
                  key={stat.label}
                  className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex items-center gap-3 shadow-sm"
                >
                ...
                <table className="w-full hidden md:table">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                      <th className="text-left py-3.5 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        Empleado
                      </th>
                      <th className="text-left py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        Rol
                      </th>
                      <th className="text-left py-3.5 px-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        Sucursal
                      ...
                      <th className="text-right py-3.5 px-6 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        Acciones
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {filtered.map((emp) => {
                      const role =
                        ROLE_CONFIG[emp.role] || ROLE_CONFIG.employee;
                      ...
                      return (
                        ...
                        >
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              <div className="size-9 rounded-full flex-shrink-0 overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                                {emp.avatarUrl ? (
                                  <img
                                    src={emp.avatarUrl}
                                    alt={emp.name}
                          ...
                          <td className="py-4 px-4">
                            <span
                              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border ${role.color}`}
                            >
                              {role.label}
                            </span>
                          </td>
                  ...
                  {filtered.map((emp) => {
                    ...
                    return (
                      <div key={emp.id} className="p-4 flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                          <div className="size-10 rounded-full flex-shrink-0 overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                            {emp.avatarUrl ? (
                              <img
                                src={emp.avatarUrl}
                                alt={emp.name}
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => openEditModal(emp)}
                              className="p-1.5 text-slate-400 hover:text-primary rounded-lg transition-all"
                            >
                              <span className="material-symbols-outlined text-[18px]">
                                edit
                              </span>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold border ${role.color}`}
                          >
                            {role.label}
                          </span>
                          <button
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div className="h-24 w-24 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
                      {avatarPreview ? (
                        <img
                          src={avatarPreview}
                          alt="Avatar preview"
                    ...
                    <label className="relative inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors overflow-hidden">
                      Subir foto
                      <input
                        type="file"
                        accept="image/*"
                    ...
                    {uploadProgress > 0 && uploadProgress < 100 && (
                      <div className="w-full rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                        <div
                ...
                {!editingEmployee && (
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      El empleado podrá cambiarla después de ingresar.
                    </p>
                  </div>
                )}
                ...
                <div>
                  ...
                  >
                    <option value="employee">
                      Empleado — Inventario de su sucursal
                    </option>
                  ...
                  <div className="mt-2 flex items-start gap-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3 border border-slate-100 dark:border-slate-800">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      {formData.role === "admin" &&
                        "Acceso completo: sucursales, empleados, inventario global y reportes."}
