...
const DraggableContainer = ({ children, className = "" }) => {
  ...
  return (
    <div
      className={`absolute inset-0 overflow-auto custom-scrollbar cursor-default lg:cursor-grab lg:active:cursor-grabbing select-none flex items-center justify-center ${className}`}
      onMouseDown={onMouseDown}
      onMouseLeave={onMouseLeave}
      onMouseUp={onMouseUp}
      onMouseMove={onMouseMove}
    >
      <div className="min-w-max min-h-max p-4 lg:p-16">{children}</div>
    </div>
  );
};
