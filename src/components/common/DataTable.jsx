...
const DataTable = ({
  ...
  const filteredData = useMemo(() => {
    ...
    // Search Filter - Busca SOLO en nombre, SKU, item, brand (no descripción)
    if (searchTerm && !disableFilter) {
      result = result.filter((item) => {
        ...
        return matchesAnyFuzzy(searchTerm, searchFields);
      });
    }
    ...
    if (sortConfig.key) {
      ...
      // Si no hay sorting específico, ordena alfabéticamente por nombre/sku/item como fallback
      result.sort((a, b) => {
        const aSort = (
          a.name ||
          a.productName ||
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
          ...
          <div className="relative w-full sm:w-80 group">
            ...
            <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none z-10">
              <span className="material-symbols-outlined text-slate-400 group-focus-within:text-primary transition-colors text-[22px]">
                search
              </span>
            </div>
            <input
                    ...
                    <tr key={`skeleton-${i}`}>
                      ...
                      {actions.length > 0 && (
                        <td className="px-8 py-5">
                          <div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-lg w-12 ml-auto"></div>
                        </td>
                      )}
                    </tr>
                  ))
                  ...
                  paginatedData.map((item, index) => (
                    ...
                    >
                      {columns.map((col) => (
                        <td
                          key={col.key}
                          className={`px-8 py-5 transition-all ${col.className || ""}`}
                        >
                          {col.render
                      ...
                      {actions.length > 0 && (
                        ...
                        >
                          ...
                          <div className="md:hidden relative inline-block text-left">
                            ...
                            {openMenuId === item.id && (
                              <div className="absolute right-0 bottom-full mb-3 w-52 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl z-50 py-2.5 overflow-hidden">
                                {actions.map((action, i) => (
                                  <button
                                    key={i}
                                    onClick={(e) => {
